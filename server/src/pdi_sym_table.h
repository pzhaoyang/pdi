 ...
 * Description:
 *
 * The symbol table for use in process debug infrastructure (PDI).
******************************************************************************/
...
IMPORT uint16    pdi_sym_group_default;
IMPORT FUNCPTR   pdi_sync_sym_add_rtn;
IMPORT FUNCPTR   pdi_sync_sym_remove_rtn;

IMPORT SYMTAB_ID pdi_sym_tbl_id;            /* system symbol table */
IMPORT SYMTAB    pdi_sym_tbl;
IMPORT pdi_hash_tbl_t  pdi_hash_tbl;
IMPORT pdi_sl_list_t   pdi_hash_list[];
...
STATUS  symSAdd (SYMTAB_ID symTblId, char *name, char *value, SYM_TYPE type, uint16 group);
STATUS  pdi_sym_add (SYMTAB_ID symTblId, char *name, char *value, SYM_TYPE type, uint16 group);
...
STATUS 	pdi_sym_init (SYMBOL *pSymbol, char *name, char *value, SYM_TYPE type, uint16 group);
SYMBOL *pdi_sym_alloc (SYMTAB_ID symTblId, char *name, char *value, SYM_TYPE type, uint16 group);
...
STATUS 	pdi_sym_find_by_value (SYMTAB_ID symTblId, uint32 value, char *name, intptr_t *pValue, SYM_TYPE *pType);
STATUS 	pdi_sym_find_by_cname (SYMTAB_ID symTblId, char *name, char **pValue, SYM_TYPE *pType);
STATUS 	pdi_sym_find_by_name_and_type (SYMTAB_ID symTblId, char *name, char **pValue, SYM_TYPE *pType,
 		      SYM_TYPE sType, SYM_TYPE mask);
STATUS 	pdi_sym_find_by_value_and_type (SYMTAB_ID symTblId, uint32 value,
 		       char *name, intptr_t *pValue, SYM_TYPE *pType,
 		       SYM_TYPE sType, SYM_TYPE mask);
STATUS 	pdi_sym_by_value_find (SYMTAB_ID symTblId, uint32 value, char **pName, intptr_t *pValue, SYM_TYPE *pType);
STATUS 	pdi_sym_by_value_and_type_find (SYMTAB_ID symTblId, uint32 value,
				       char **pName, intptr_t *pValue,
				       SYM_TYPE *pType,SYM_TYPE sType,
				       SYM_TYPE mask);

STATUS pdi_sym_find_symbol( SYMTAB_ID symTblId, char * name, void * value,
                      SYM_TYPE type, SYM_TYPE mask, SYMBOL_ID * pSymbolId );
